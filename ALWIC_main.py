...
##################METHODS USED FOR PROCESSING##################################
    ...
    def processing(self):
        ...
        if self.option_protocol.get() == "van Geldern mode":
            ...
            self.final_value_file_df,self.calibration_param_list,self.calibration_vectors=cal.wrapper_calibration(self.corrected_file_df, self.iso_type_list, self.std_idx_norm, self.std_values, self.inj_per_std, self.result_file_df, self.removed_inj_per_std,self.std_nbr)
            self.slope_MC_list, self.p_values_MC_list, self.avg_std_list, self.std_dev_std_list, self.std_col1_list, self.residuals_std, self.std_uncheck, self.spl_results, self.known_sample_results=param_calc.wrapper_parameters_calculation(self.final_value_file_df, self.protocol_type, self.removed_inj_per_std, self.std_nbr, self.inj_per_std, self.iso_type_list, self.is_residuals_results_table, self.is_spy_results_table, self.var_checkbox_std_table_dict, self.std_values, self.removed_inj_per_spl, self.spl_nbr, self.inj_per_spl, self.len_std_injections, self.port_known_samples_list, self.idx_known_sample)
